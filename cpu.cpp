/*
This gameboy emulator was developped as an exercise into becoming
a better programmer.
...
namespace cpu{
    ...
    uint8_t instruction_fetch(uint16_t &pc){
         uint8_t retVal = read(pc);
         pc++;
         return retVal;
    }
    ...
    void run_frame(){
        cycles = 0;
        opcode = instruction_fetch(pc);
        do{
            if(opcode == 0xCB) {cycles += execute_cb_opcode(instruction_fetch(pc));}
            else{ cycles += execute_normal_opcode(instruction_fetch(pc));}
        }while(cycles <= 70224);
    }
    ...
    uint8_t execute_normal_opcode(const uint8_t opcode){
        switch(opcode){
    ...
    uint8_t execute_cb_opcode(const uint8_t opcode){
        switch(opcode){
        ...
        return opcode_cybles_cb[opcode];
    }
